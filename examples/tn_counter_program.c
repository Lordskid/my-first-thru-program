...
static void handle_create_counter(uchar const *instruction_data) {
    tn_counter_create_args_t const *args =
        (tn_counter_create_args_t const *)instruction_data;

    ushort account_idx = args->account_index;

    uchar const *proof_data = 0;
    ...
    tn_counter_account_t *counter_account =
        (tn_counter_account_t *)tsdk_get_account_data_ptr(account_idx);
...
static void handle_increment_counter(
...
TSDK_ENTRYPOINT_FN void start(void) {
    ...
    uchar const *instruction_data =
        tsdk_txn_get_instr_data(txn);

    ulong instruction_data_sz =
        tsdk_txn_get_instr_data_sz(txn);
    ...
    switch (*instruction_type) {
        ...
        case TN_COUNTER_INSTRUCTION_CREATE: {
            ...
            if (instruction_data_sz <
                sizeof(tn_counter_create_args_t)) {
                tsdk_revert(
            ...
            handle_create_counter(instruction_data);
            break;
        }
        ...
        case TN_COUNTER_INSTRUCTION_INCREMENT:
            ...
            if (instruction_data_sz !=
                sizeof(tn_counter_increment_args_t)) {
                tsdk_revert(
                    TN_COUNTER_ERR_INVALID_INSTRUCTION_DATA_SIZE
                );
            }

            handle_increment_counter(instruction_data);
            break;
